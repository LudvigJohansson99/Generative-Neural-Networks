...
dictionary = torch.load("model_weights_tanh.pth")
...
class Generator(nn.Module):
    def __init__(self, input_dimension=128, device="cpu"):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Upsample(scale_factor=2, mode='nearest'),  # Upsample to 128x128
            nn.Conv2d(in_channels=32, out_channels=16, kernel_size=3, padding=1),
            nn.LeakyReLU(),
            
            nn.Conv2d(in_channels=16, out_channels=3, kernel_size=3, stride=1, padding=1),  # Reduce to 124x124
            nn.Tanh()  # Assuming output values between 0 and 1
        )
...
for e in range(nr_epochs):
    ...
    grid_img_pil.save("GAN" + str(e) + ".png")
...
grid_img = torchvision.utils.make_grid(samples, nrow=8)
grid_img_pil = torchvision.transforms.ToPILImage()(grid_img)
grid_img_pil.save("GAN" + str(nr_epochs) + ".png")
